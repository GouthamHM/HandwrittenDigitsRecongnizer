...
def softmax_cross_entropy_loss_der(Y, cache):
    ...
    '''
    ### CODE HERE 
    one_hot = np.eye(cache['Z'].shape[0])[Y.astype(np.int64).reshape(-1)].T
    Z = cache['Z']
    A = np.exp(Z - np.amax(Z, axis=0)) / np.sum(np.exp(Z - np.amax(Z, axis=0)), axis=0)
    dZ = A-one_hot
    return dZ
...
def initialize_multilayer_weights(net_dims):
    ...
    for l in range(numLayers-1):
        ...
        parameters["b"+str(l+1)] = np.random.randn(net_dims[l+1],1)*0.01#CODE HERE
    return parameters
...
def main():
    ...
    plot_cost(costs,len(net_dims)-2,trAcc,teAcc)
